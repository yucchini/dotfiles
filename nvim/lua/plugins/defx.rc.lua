vim.api.nvim_set_keymap('n', 'fs', ':<C-u>Defx -listed -resume -columns=indent:mark:icon:icons:filename:git:size -buffer-name=tab`tabpagenr()`.`expand("%:p:h")` -search=`expand("%:p")`<CR>', { noremap = true, silent = true })
vim.api.nvim_set_keymap('n', 'sf', ':<C-u>Defx -new -columns=indent:mark:icon:icons:filename:git:size -buffer-name=tab`tabpagenr()`.`expand("%:p:h")` -search=`expand("%:p")`<CR>', { noremap = true, silent = true })
